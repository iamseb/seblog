defmodule Seblog.TemplatedView do
  @moduledoc """
  ...
  defmacro __using__(opts) do
    quote do
      @template_root String.replace(Keyword.fetch!(unquote(opts), :root), "/*", "")
      import Seblog.TemplatedView
      ...
      def render(template, assigns) do
        IO.puts "USING TEMPLATEDVIEW"
        module = Phoenix.Template.module_to_template_root(__MODULE__, Seblog, "View")
        theme = Map.get(assigns, :theme, "default")
        template = Path.join([@template_root, theme, module, template])
        # template = "default/" <> template

        IO.puts "Calling template #{template} with assigns:"
        IO.inspect assigns
      ...
      def render_existing(module, template, assigns \\ []) do
        IO.puts "USING TEMPLATEDVIEW RENDEREXISTING"
        module = Phoenix.Template.module_to_template_root(__MODULE__, Seblog, "View")
        theme = Map.get(assigns, :theme, "default")
        new_template = Path.join([@template_root, theme, module, template])
        render(module, new_template, put_in(assigns[:render_existing], {module, new_template}))
      end
